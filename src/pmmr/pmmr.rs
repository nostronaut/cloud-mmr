...
use crate::pmmr::{Backend, ReadonlyPMMR};
...
pub trait ReadablePMMR {
    ...
    /// Computes the specify range height root of the MMR. Find all the peaks in the specify
    /// tree and "bags" them to get a single peak.
    fn root_at(&self, pos0: u64) -> Result<Hash, String> {
        if self.is_empty() || pos0 == 0 {
            return Ok(ZERO_HASH);
        }
        if !is_leaf(pos0) {
            return Err("pos0 not leaf".to_owned());
        }
        if pos0 > self.unpruned_size() {
            return Err("pos0 > unpruned_size".to_owned());
        }
        let mut res = None;
        let peaks: Vec<Hash> = peaks(pos0)
            .into_iter()
            .filter_map(move |pi0| self.get_peak_from_file(pi0))
            .collect();
        for peak in peaks.into_iter().rev() {
            res = match res {
                None => Some(peak),
                Some(rhash) => Some((peak, rhash).hash_with_index(pos0)),
            }
        }
        res.ok_or_else(|| "no root, invalid tree".to_owned())
    }
